...
1. **回归测试是唯一验收标准**：改完 index.html 必须跑
   `node tests/regression.js`（零依赖，**60 项断言**）。绿灯才允许 commit。
   覆盖：内联脚本语法 / 启动骨架完整性 / 刷题列表优先未答且一轮内不重排 /
   错题本 Leitner 升降盒与毕业 / **题库数据体检** / **答题推进与边界**。
   **不要再写用完就删的临时测试脚本**，要加断言就加进 `tests/regression.js`。
...
**🧪 逻辑测试盖不住数据 bug（2026-09-03 立）**：答案字母越界 / 选项重复 / 题干重复
这类「题目本身的 bug」**不会让页面报错**，只会让人做题时看到错的东西，
逻辑断言永远抓不到。所以 `tests/regression.js` 组6 对 `QUIZ`(247) 与 `RK_QUIZ`
**逐题体检**（字段完整 / 选项≥2项无空串 / 同题内无重复选项 / **答案字母必须落在
选项范围内** / 有解析 / 无重复题干 / 每域≥5 题）。首跑即抓出第 77 题
「逆向工程」C、D 选项一字不差重复。**新增题目必过这组体检。**

**棘轮式管理存量欠债**：无法一次修完的质量问题（如 91/247 题的 `e` 只有出处署名、
无讲解）用「基线常量 + 只许降不许升」处理——超基线即 `exit 1` 拦新增，
存量用 `warn()` 打 ⚠️ 但不阻断部署。避免二选一：要么阻塞正常改动、要么放任腐化。

**判"有没有内容"别用长度阈值**：初版用「剥署名后 <8 字符即无解析」，
误伤 `全生命周期管理。`（7 字真解析），还把非题目对象的 `e` 字段
（`inspired-public`/`datafield`/`高分精学` 等资料条目）算进来 → 报 100 而非 91。
**正确做法：按分隔符切段 + 丢掉匹配署名正则的段 + 一段不剩才算空**，
且只遍历 `QUIZ` 数组、不 grep 全文件。
